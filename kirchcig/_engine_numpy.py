...
arithmetic exactly: traveltimes, the interpolation index and weights, and the
angle bin are all computed in float32 with the same operation order as the
kernels, and the sums are accumulated in float64 (``np.bincount``). The two
engines therefore agree to float32 rounding of the final result.
...
class NumpyEngine:
    ...
    def _pairs(self, s, r0, r1):
        ...
        float32 weight of the second tap, and the gather bin."""
        ...
        return rr, pp, it[rr, pp], w[rr, pp], h[rr, pp]
    ...
    def adjoint(self, data):
        ...
        for s in range(self.ns):
            ...
            for r0 in range(0, self.nr, self.rchunk):
                ...
                rr, pp, itv, wv, hv = self._pairs(s, r0, r1)
                if rr.size == 0:
                    continue
                w1, w2 = self._taps(wv)
                rg = r0 + rr
                v = ds[rg, itv].astype(np.float64) * w1 + ds[rg, itv + 1].astype(np.float64) * w2
