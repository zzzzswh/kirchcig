...
- `aa_stretch`（默认开）同时覆盖成像网格单元的时间足迹 `|∂τ/∂x|·dx + |∂τ/∂z|·dz`。一个成像样点代表一个 `dx × dz` 的单元；深度网格粗于时间采样（`2·dz/v > dt`）时，直接求和的每个深度样点落在相隔好几个时间样点的位置，反偏移出来的反射层是一排插值脉冲。这就是 Madagascar 的 `aastretch`。炮点、检波点两侧的梯度先求和再取绝对值，所以镜面反射点处足迹只剩深度项；两种宽度按均方根合并。代价是每项贡献多读一次 8 字节的梯度表；`aa_stretch=False` 退回 `sfmig2` 那种纯道轴判据。
...
算子构建（含走时表与一次性 NVRTC 编译）约 1.4 s（`aa=True` 时 2.0 s，多出的是对走时表差分算算子倾角）。抗假频在伴随上约 1.5 倍开销（每项贡献读 3 个 float64 抽头而不是 1 个 float32 样点），正演 2.6 倍（6 次共享内存原子加代替 2 次，加上 float64 输出及其反向积分）。默认开着的 `aa_stretch` 项每项贡献再多读一个 8 字节的梯度表元素，伴随再慢 1.7 倍——这说明伴随受表读取而不是算术限制，把梯度打包进走时表元素是下一个明显的优化点。这台机器不同 session 之间计时差可达 25%（`aa_stretch=False` 那行另一天测得 98.5 ms），比较时看同一次测的几行。`forward` 比 `adjoint` 快近一倍：它每个 block 处理一个道，在共享内存里累加完一次写出；而 `adjoint` 要沿走时曲线做不规则的散射读取。
...
复现：`python benchmarks/bench.py`，可接受 `--acc float32`、`--nh`、`--domain`、`--aa`、`--no_aa_stretch`、`--aperture`、`--halfderiv`、`--weight`、`--engine numpy` 等参数。前两行是不开抗假频的结果。算子在单张卡上运行，可通过 `cupy.cuda.Device` 指定设备；使用 PyTorch 封装时则由张量所在设备决定。
