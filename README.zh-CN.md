...
| `float64`，`aa=True, aa_stretch=False` | 77.3 ms — 20.8 G 配对/秒 | 67.3 ms — 23.9 G 配对/秒 | 2.0e-08 |
| `float64`，`aa=True`（含 `aa_stretch`） | 128.8 ms — 12.5 G 配对/秒 | 72.0 ms — 22.4 G 配对/秒 | 2.3e-08 |

算子构建（含走时表与一次性 NVRTC 编译）约 1.4 s（`aa=True` 时 2.0 s，多出的是对走时表差分算算子倾角）。抗假频在伴随上约 1.5 倍开销（每项贡献读 3 个 float64 抽头而不是 1 个 float32 样点），正演 2.6 倍（6 次共享内存原子加代替 2 次，加上 float64 输出及其反向积分）。`aa_stretch` 那行是梯度还单独占一次 8 字节读取时测的，那时它让伴随慢 1.7 倍，说明伴随受表读取限制；此后梯度已打包进表元素，伴随也改成每个检波点元素复用 `schunk` 个炮点（`--schunk`），这几行应该会降下来（尚未重测）。这台机器不同 session 之间计时差可达 25%（`aa_stretch=False` 那行另一天测得 98.5 ms），比较时看同一次测的几行。`forward` 比 `adjoint` 快近一倍：它每个 block 处理一个道，在共享内存里累加完一次写出；而 `adjoint` 要沿走时曲线做不规则的散射读取。
