...
- `aa_stretch`（默认开）同时覆盖成像网格单元的时间足迹 `|∂τ/∂x|·dx + |∂τ/∂z|·dz`。一个成像样点代表一个 `dx × dz` 的单元；深度网格粗于时间采样（`2·dz/v > dt`）时，直接求和的每个深度样点落在相隔好几个时间样点的位置，反偏移出来的反射层是一排插值脉冲。这就是 Madagascar 的 `aastretch`。炮点、检波点两侧的梯度先求和再取绝对值，所以镜面反射点处足迹只剩深度项；两种宽度按均方根合并。梯度随走时表元素一起读（offset 域每个成像点 16 字节，带角度或权重时 32 字节）；`aa_stretch=False` 退回 `sfmig2` 那种纯道轴判据。
...
一条道只对同时落在其炮点**和**检波点孔径内的成像点有贡献：在它们正下方 `aperture` 度的锥内（`sfkirmig` 的 `aperture=`），或横向距离不超过 `apt` 米（`sfmig2` 的 `apt=`，这里单位是米）。这既压远端摆动噪声，又跳过被丢弃贡献的计算。README 那个几何下 60° 锥保留 47% 的道–成像点对（45°：23%）；V100 上正演从 25.5 ms 降到 17.2 ms，伴随则完全没有变快——它的 block 是一段深度列，横跨锥面边缘，被掩掉的线程只能等同一 warp 里其他线程做完 gather，而且每个 pair 的合并表读取照样要做。所以把孔径当作一个附带正演加速的成像控制项来用。和 Madagascar 一样是硬截断；`op.aperture_masks()` 返回两个布尔掩码，`python benchmarks/bench.py --aperture 60` 会打印保留比例和计时。
...
```python
op = KirchhoffCIG(..., weight="obliquity")                     # 每侧 sqrt(cos θ)
op = KirchhoffCIG(..., weight=["obliquity", "spreading"])      # 再乘每侧 1/sqrt(t)
op = KirchhoffCIG(..., weight=lambda t, theta, dt: np.cos(theta) ** 2)
op = KirchhoffCIG(..., weight=(w_srcs, w_recs))                # (ns, nx, nz), (nr, nx, nz)
```

每项贡献乘 `w_s(炮, x, z) · w_r(检, x, z)`，即炮侧表与检侧表的乘积。预设有 `"obliquity"`（`sqrt(cos θ)`，乘积是两侧出射角余弦的几何平均，也就是克希霍夫正演的倾斜因子；`sfkirmod` 用算术平均，二阶以内一致）和 `"spreading"`（`1/sqrt(t)`，单程 2D 格林函数的幅度，差一个速度常数）；给列表就相乘；给可调用对象就在每侧的走时表和出射角表上求值；给一对数组就直接用。两个内核乘的是同一个 float32 乘积，所以 `forward` 是 `A·W`、`adjoint` 是 `W·Aᵀ`，算子对仍是精确转置。权重随走时表元素一起读，不多花一次访存。这些覆盖了倾斜与扩散因子；完整的真幅度（Bleistein）权不能拆成两侧的乘积，没有提供。
...
代价是每道一次 float64 FFT，在引擎所在设备上运行，与内核无关。开着它 `dot_test()` 照样通过。两点要知道：离散滤波器带四分之一个样点的延迟（相位是 `π/4 − ω/4`），所以偏移出来的反射层在双程时间上浅 `dt/4`，往返一次则没有位移；它管不了深度网格的梳状问题（`2·dz/v > dt`），那个由 `aa=True` 负责。
...
|---|---|---|---|
| `float64`（默认） | 33.9 ms — 47.5 G 配对/秒 | 25.6 ms — 63.0 G 配对/秒 | 9.5e-08 |
| `float32` * | 33.5 ms — 48.2 G 配对/秒 | 18.8 ms — 85.8 G 配对/秒 | 1.1e-07 |
| `float64`，`aa=True, aa_stretch=False` | 72.7 ms — 22.2 G 配对/秒 | 67.3 ms — 23.9 G 配对/秒 | 2.0e-08 |
| `float64`，`aa=True`（含 `aa_stretch`） | 62.0 ms — 26.0 G 配对/秒 | 71.2 ms — 22.6 G 配对/秒 | 2.3e-08 |

<sub>* `float32` 这行是伴随加入炮点分组之前测的，表里其余伴随数字都已随之改变；以 `float64` 几行为准。</sub>

算子构建（含走时表与一次性 NVRTC 编译）约 1.4 s，`aa=True` 时 2.1 s，再加 `aa_stretch` 2.6 s（多出的是对走时表差分算算子倾角）。抗假频在伴随上是 1.8 倍开销（每项贡献读 3 个 float64 抽头而不是 1 个 float32 样点），正演 2.8 倍（6 次共享内存原子加代替 2 次，加上 float64 输出及其反向积分）。这台机器不同 session 之间计时差可达 25%（`aa_stretch=False` 那行另一天测得 98.5 ms），比较时看同一次测的几行。`forward` 原本比 `adjoint` 快近一倍——它每个 block 处理一个道，在共享内存里累加完一次写出，而 `adjoint` 要沿走时曲线做不规则的散射读取——但伴随的炮点分组把这个差距基本抹平了，开 `aa=True` 时正演反而成了慢的那个。

`--schunk` 控制伴随在寄存器里同时握住几个炮点，使每个检波点表元素一组只读一次而不是每个炮点读一次。同一 session 内扫最后一行：`--schunk 1` 118.1 ms，默认 4 是 62.0 ms，8 是 59.5 ms，正演不受影响，恒为 71.1 ms。这 1.9 倍就是这一行从 0.2.0 的 128.8 ms 降下来的主要来源；把梯度打包进走时表元素只占其中较小的一段（`--schunk 1` 下 128.8 → 118.1 ms，跨 session 测的）。有一个结果还没有解释：打包加分组之后，`aa_stretch` 比纯道轴判据还要快——62.0 对 72.7 ms——而它的表元素宽一倍；同时 `aa_stretch=False` 那行本身还没有扫过 `schunk`。在补上之前，这两行的伴随数字应当看作大致相同，而不是 stretch 项的实测代价。

角度域会把元素补到最宽的 8 个 float：走时、出射角、算子倾角和两个梯度分量。同一 session 内 `--domain angle --aa` 伴随 91.1 ms、正演 113.7 ms，dot-test 误差 8.4e-08。
...
复现：`python benchmarks/bench.py`，可接受 `--acc float32`、`--nh`、`--domain`、`--aa`、`--no_aa_stretch`、`--aperture`、`--halfderiv`、`--weight`、`--schunk`、`--engine numpy` 等参数。前两行是不开抗假频的结果。算子在单张卡上运行，可通过 `cupy.cuda.Device` 指定设备；使用 PyTorch 封装时则由张量所在设备决定。
...
- **累加器默认 float64。** 一个偏移样点要累加 10^4 到 10^5 项。用 float64 累加时，CUDA 引擎与 NumPy 参考引擎逐位一致；改用 float32 约引入 1e-7 相对误差，以前能换来伴随约 1.5 倍的加速（炮点分组之后尚未重测）。
- **每个（侧，成像点）一个表元素。** 走时、出射角、算子倾角、幅度权、走时梯度按固定顺序打包成 1、2、4 或 8 个 float，所以不管开什么选项，一项贡献每侧只花一到两次对齐的 16 字节读取。检波点表是伴随最主要的访存流（放不进 L2，每个炮点都要重读一遍），因此伴随按 `schunk`（默认 4）个炮点一组放在寄存器里，每个检波点表元素一组只读一次。
...
- **没有真幅度权。** `weight=` 覆盖可分离的因子（倾斜、扩散，任何 `w_s·w_r` 形式）；把偏移从伴随变成逆的 Bleistein/Schleicher 权不在其中。
- **不开 `aa=True` 时正演不对深度→时间的拉伸做抗假频。** `2·dz/v > dt` 时直接求和的反偏移道是一排插值脉冲；要么取 `dz ≤ v·dt/2`，要么开 `aa`（它的 `aa_stretch` 项负责这个）。
- **仅支持 2D。** 瓶颈在走时表，不在核函数。
